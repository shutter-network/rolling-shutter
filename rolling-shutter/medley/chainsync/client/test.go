...
func (c *TestClientController) AppendNextHeader(h *types.Header, events ...types.Log) {
	...
	if ok {
		//FIXME: this happens now we push all chainsegments from the makesegmetn...
		// when can this happen? just return?
		// panic("hash for header already present in log cache")
		return
	}
