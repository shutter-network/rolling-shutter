...
func TestStateTransition(t *testing.T) {
	...
	spamStateTransition := func(b *batch.Batch, epochChan chan<- time.Time, tx, epoch, decryption, confirmation bool) {
		...
		// We can't spam the state-transition with a previous batches
		// state update in black-box testing 	because this would require access to private
		// properties
	...
	headBatchSubscription := make(chan batch.StateChangeResult, 1)
	nextBatchSubscription := make(chan batch.StateChangeResult, 1)
	...
	// now run the 2 successive batches Run function
	// this will start the state-change broker
	// and the state-transition function
	egroup.Go(func() error {
		return headBatch.Run(epochChan1, headBatchSubscription)
	})
	egroup.Go(func() error {
		return nextBatch.Run(epochChan2, nextBatchSubscription)
	})
