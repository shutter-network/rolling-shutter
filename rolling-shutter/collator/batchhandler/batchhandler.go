...
func NewBatchHandler(cfg config.Config, dbpool *pgxpool.Pool) (*BatchHandler, error) {
	...
	bh := &BatchHandler{
		...
		ticker:                 StartEpochTicker(cfg.EpochDuration),
		confirmedBatchesBroker: medley.NewBroker[epochid.EpochID](0, true),
...
func (bh *BatchHandler) Run(ctx context.Context) error {
	...
	// to the batches.
	go bh.confirmedBatchesBroker.Start()
	defer bh.confirmedBatchesBroker.Stop()
...
func (bh *BatchHandler) HandleStateTransitions(ctx context.Context, eg *errgroup.Group, b *batch.Batch) error {
	...
	subscription := make(chan batch.StateChangeResult, 1)
	confirmedBatch := bh.confirmedBatchesBroker.Subscribe()
	epochTick := bh.ticker.Subscribe()
	go b.Run(epochTick, subscription)

	stop := func() {
		b.Broker.Unsubscribe(subscription)
		bh.removeBatch(b)
	...
	for {
		...
		case stateTransition, ok := <-subscription:
			if !ok {
				subscription = nil
				continue
			}
...
func (bh *BatchHandler) HandleBatchConfirmation(epochID epochid.EpochID) error {
	bh.confirmedBatchesBroker.Publish(epochID)
	return nil
}
