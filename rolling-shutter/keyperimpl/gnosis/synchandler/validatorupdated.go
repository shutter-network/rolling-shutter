...
func (kb *ValidatorUpdated) ABI() abi.ABI {
	return *kb.evABI
}
func (kb *ValidatorUpdated) Accept(
	ctx context.Context,
	header types.Header,
	event bindings.ValidatorregistryUpdated,
) (bool, error) {
	return true, nil
}
...
func (vu *ValidatorUpdated) filterEvents(
	...
	for _, event := range events {
		...
		if !vu.checkStaticRegistrationMessageFields(msg, event.Raw.Address, msg.ValidatorIndex) {
			continue
		}
...
func (vu *ValidatorUpdated) checkStaticRegistrationMessageFields(
	...
	// TODO: was passed in from outside
	// logger := evLog.With().Uint64("validator-index", msg.ValidatorIndex).Logger()
