...
// Hash: b2d4d07c47c97cb6bfcab12c8875992def8cafc76eb785dff329e8b6795da613
// Version: 0.1.4
...
func (i *IdentityPreimage) MarshalSSZTo(buf []byte) (dst []byte, err error) {
	...
	// Field (0) 'Bytes'
	if size := len(i.Bytes); size != 32 {
		err = ssz.ErrBytesLengthFn("IdentityPreimage.Bytes", size, 32)
		return
	}
...
func (i *IdentityPreimage) UnmarshalSSZ(buf []byte) error {
	...
	// Field (0) 'Bytes'
	if cap(i.Bytes) == 0 {
		i.Bytes = make([]byte, 0, len(buf[0:32]))
	}
	i.Bytes = append(i.Bytes, buf[0:32]...)
...
func (d *DecryptionSignatureData) UnmarshalSSZ(buf []byte) error {
	...
	// Field (2) 'IdentityPreimages'
	{
		...
		for ii := 0; ii < num; ii++ {
			if err = d.IdentityPreimages[ii].UnmarshalSSZ(buf[ii*32 : (ii+1)*32]); err != nil {
				return err
			}
		}
	}
...
func (d *DecryptionSignatureData) SizeSSZ() (size int) {
	...
	// Field (2) 'IdentityPreimages'
	size += len(d.IdentityPreimages) * 32
