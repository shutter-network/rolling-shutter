...
func (h *EpochKGHandler) validateDecryptionKey(ctx context.Context, key *p2pmsg.DecryptionKey) (bool, error) {
	if key.GetInstanceID() != h.config.GetInstanceID() {
		return false, errors.Errorf("instance ID mismatch (want=%d, have=%d)", h.config.GetInstanceID(), key.GetInstanceID())
	}
	...
	dkgResultDB, err := kprdb.New(h.dbpool).GetDKGResult(ctx, int64(key.Eon))
...
func (h *EpochKGHandler) validateDecryptionKeyShare(ctx context.Context, keyShare *p2pmsg.DecryptionKeyShare) (bool, error) {
	if keyShare.GetInstanceID() != h.config.GetInstanceID() {
		return false, errors.Errorf("instance ID mismatch (want=%d, have=%d)", h.config.GetInstanceID(), keyShare.GetInstanceID())
	}
	...
	dkgResultDB, err := kprdb.New(h.dbpool).GetDKGResult(ctx, int64(keyShare.Eon))
	if err == pgx.ErrNoRows {
...
func (h *EpochKGHandler) validateDecryptionTrigger(ctx context.Context, trigger *p2pmsg.DecryptionTrigger) (bool, error) {
	if trigger.GetInstanceID() != h.config.GetInstanceID() {
		return false, errors.Errorf("instance ID mismatch (want=%d, have=%d)", h.config.GetInstanceID(), trigger.GetInstanceID())
	}
